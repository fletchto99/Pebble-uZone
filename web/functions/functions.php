...
class Functions {
    ...
    private $result = ['error' => 'Username or password not configured correctly.'];
    ...
    function execute($method) {
        if ($this->username && $this->password) {
            switch ($method) {
                case 'CheckLogin':
                    $form = ['ID' => $this->config['ID'], 'name' => $this->username, 'pass' => $this->password, 'optStudent' => 'optStudent'];
                    $login = self::checkLogin($form, $this->config['LOGIN_URL']);
                    $this->result = ['login' => ($login ? 'success' : 'failed')];
                    break;
                case 'MealPlanCheck':
                ...
                case 'Balance':
                    ...
                    break;
                case 'MealHistory':
                    $meal = new MealHistory($this->username, $this->password, $this->config['ID'], $this->config['MEAL_HISTORY_URL']);
                    $this->result = $meal->execute();
                    break;
                case 'FlexHistory':
                    $flex = new FlexHistory($this->username, $this->password, $this->config['ID'], $this->config['FLEX_HISTORY_URL']);
